...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    // The shell exits immediately, but its descendant keeps the PTY output
    // descriptors open. Unified exec must not wait for that descendant before
    // publishing the command's completion.
    let args = json!({
        "cmd": "(sleep 5) & printf 'HELLO-FULL-LIFECYCLE'",
        "yield_time_ms": 1000,
    });
...
#[cfg(target_os = "macos")]
#[allow(clippy::expect_used)]
async fn macos_seatbelt_denial_test(server: &wiremock::MockServer) -> Result<TestCodex> {
    let home = Arc::new(TempDir::new()?);
    fs::write(
        home.path().join("config.toml"),
...
async fn unified_exec_logs_macos_seatbelt_denials() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let test = macos_seatbelt_denial_test(&server).await?;
